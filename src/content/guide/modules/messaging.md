...
### Details  
Setting your email details is a crucial first step to sending a successful email. Write your message subject, and decide whether or not to include the Message Prefix (set in the Admin section).  
...
#### Delivery Score  
The Delivery Score will change as you update the text in the Message Subject and/or Message Preview. It will give you suggestions of how to better improve your send-score.  
...
Any content relevant to less than 75% of your audience: it's better to send it in a separate email and filter your recipient list to the people it will be most relevant to.  
...
You can also send this as a preview email to yourself or a select number of individuals from the bottom of this page, and duplicate your message. See more on this below.
...
Fresh Vine utilizes a third party email sending service that ensures every message is correctly signed, verified, and sent. After the message is sent our system stores a verification key that we can use to track back any abuse/spam notifications. This same verification key allows us to know that an email was successfully sent (though does not guarantee delivery). These keys are only available to select members of our staff.  
  
## Email Settings  

### Subject  
The subject is what will be used for the subject line of your message. This is the very first thing your recipients will read of your message.  

### Message Preview
After the subject, this is the text your recipients will read next. This should be explicit about the purpose and content of your email. It appears in the very top of your message at the very top of your template. When received on a mobile phone, or in most modern email clients this preview text appears immediately below the subject.  
  
In addition this preview text is also used as the default social sharing message with publicly visible messages. When you click to share on Facebook, or to tweet the public link to this message the preview text is what is automatically filled in for the user.  

### Default Message Prefix  
It can be helpful to have a prefix applied to all your email subjects unless explicitly disabled. When defined (on the email settings page) it will be prepended to the subject line for any message that does not disable this option. This will transform this subject `Potluck Fundraiser Friday at 7pm` into `MN-Vipers: Potluck Fundraiser Friday at 7pm` where `MN-Vipers` is the Message Prefix defined on the email settings page.  
  
**Tip:** The shorter the better, if it is too long it will push your subject out of view on many mobile devices lowering your open rate.  

### Visibility  
Each message can be visible or not. It is not possible to send a public message from a private group. Therefore all messages from private groups are automatically marked as private. If it is visible your message will:
  
* 	Appear on the public email list at https://*yoursite*.freshvine.co/email/ 
* 	Have a functional public link to read the message online 
* 	Include Social Sharing buttons when viewed online
* 	Be accessible by people inside and outside of your community online
  
Private messages will appear in a personalized message history for the individuals who were included in the recipient list. This is the only setting which can be altered after the message as been sent.  
  
### From Email Address
The name on this email is what will appear in the recipients inbox. The address that you select must have previously been verified. If there are no verifiable options then your message cannot be sent.

### Reply-To Email Address  
If and when a recipient chooses to reply to a message you send this address specifies to whom that message will be delivered. This can be helpful if you send an email from your organization asking for volunteers, then all the replies can go to the individual who manages those volunteers.  This address must also have previously been verified. If there are no verifiable options your message cannot be sent.  
  
### Content Layout
The content layout allows defines the number of rows and types of columns for your email. Each column has an image space at the top, and a text space underneath. You do not need to fill both. Once a content layout is selected you cannot change it.

### Email Template
This is the graphical wrapper around your content. Fresh Vine comes preloaded with a couple options that you can use, customize, or duplicate. You can also make your own. The template you select will 

### Filter  
The filter allows you to target the audience for your message. The filter assumes a group if you are sending a group message. There may be messages that are tailored for a specific audience and the filter allows you to target them easily. The second step of the filter is focused list. This is a list of everyone who met the criteria of your filter. It will show those who will not receive your message (no emails, bad emails, unsubscribed), and it allows you to exclude recipients individually.

### Removing a Message  
You can only remove a message before it has been sent. Next to each of the `Update` buttons is one that is marked as `Remove`. Pressing the `Remove` button will delete your draft. Drafts are also automatically removed if they have not be modified in 90 days. This is done to remove clutter from your email dashboard and to dispose of messages that were abandoned. Once a message was sent it is not possible for it to be removed. This ensures that there is an accurate history of messages a person has received from your organization.
...
The message itself gets duplicated. The Message Subject, Message Preview, the Content/body and images in the body.
...
Pre-formatted Content is different than a Template. An Email Template sets how the email will look - colors, fonts, backgrounds, etc. *Pre-formatted Content* is content that you'd like to have in many (or each) of your messages. It is generally (but not always) content that doesn't change, or may change just slightly depending on the particular message being sent.  
  
An example of this: You may have a monthly email that goes out, with a budget update. The budget update gets put in every one of these emails. It explains where funds go to, and how far along the community is, with raising, or keeping up with the appropriate funds. Rather than writing the same thing out for each message, you can write it once in the form of Pre-formatted Content. Now when you go to send the message, all you need to do is open the text editor when writing your email, click the Dynamic Content button and select which content to add to your message.  ![Preformatted Content button](/assets/images/email/Dynamic-Content.png) You can then make the appropriate changes to the Pre-formatted Content. In this case, you would most likely be updating the budget numbers.  

## Email Address Verification  
Any email address (Organization, Group, and Leader site Users) that will be sending messages on your behalf must first be verified. Verification is simple. An email is sent to your email address with a link you must click. By clicking the link you verify that you are in control of that email address. Internally we log the time and IP address that verifies each email address.  
    
If your account has unverified email addresses you will be able to resend the validation email from the home dashboard in your leaders site.

You may not need to verify the email address for your user account if you finalize the account by clicking the link in the email sent to you. By opening the page to set your password you have also verified your control of that email address.  
